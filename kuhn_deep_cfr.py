...
def deep_gxfp(T, N):
    ...
    adv0 = deque(maxlen=40000000)
    adv1 = deque(maxlen=40000000)

    strat = deque(maxlen=40000000)
    ...
    for t in range(T):
        ...
        print('Average utility:', avg_util)
...
strat, util, avg_util, cfr_util_list = deep_gxfp(500, 1000)

import numpy as np
util_array = np.array(cfr_util_list)
np.save('C:/Users/Vik/GameTheory/Deep-GXFP/deep_gxfp_utility', util_array)
cfr_every_100 = []
...
x = np.array(avg_util)
plt.plot(x)
plt.title('Exploitability of Kuhn Poker with Deep CFR')
